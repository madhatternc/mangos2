...
class Parser:
    ...
    def parseMultiLineScalar(self, value):
        if value == '>':
            ...
            yaml.stream.SKIP_COMMENTS = False
            i = joinLiteral(self.parseBlock())
            yaml.stream.SKIP_COMMENTS = True
            self.nestedDocs.commentEater.eatComments()
            return (1, i)
        elif value == '|+':
            yaml.stream.SKIP_COMMENTS = False
            i = joinLiteral(self.unprunedBlock())
            yaml.stream.SKIP_COMMENTS = True
            self.nestedDocs.commentEater.eatComments()
            return (1, i)
