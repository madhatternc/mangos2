...
// The search key type: it is actually a POINTER to the value
...
protected:
    ...
    /**
     * This is a highly optimized vector for holding sorted buckets.
...
#define IS_LARGE_BUCKET(el0) \
    ...
    /**
     * Resize the hash vector so that it uses so much buckets.
     * The hash vector really adjusts its size automatically, so there's
     * no need to call this function.
     * @arg iBuckets
     *   Number of buckets used. MUST BE A POWER OF TWO.
     */
...
#define MVS_SMALL   3
     ...
     * @arg MvsS
     *   Memory usage vs Speed. You can somewhat modify the behaviour of the
     *   hash vector by setting this to a value different that default.
     ...
     * @arg HashBits
     *   This parameter defines how much bits to use from the hash function.
     *   Basically this defines how much buckets there are initially: 2^HashBits.
     *   The larger you expect the amount of items, the larger numbers you should
     *   use here. The default value is good for hash tables of around 4000 elements;
     *   the optimal value can be approximatively computed as
...
public:
    ...
    /**
     * Iterator will help you traverse the hash vector items back and forth.
     */
    class Iterator
    {
        ...
        // Private constructor to initialize an iterator pointing to arbitrary place
        Iterator (const HashVector *parent, int curbucket, int delta)
        { Parent = parent; CurrentBucket = curbucket; BucketDelta = delta; }

    public:
        // Get the hash vector element pointed by this iterator
        Some operator * ()
        {
        ...
        // Get the current element or NULL and move forward
        Some Next ()
        { return *(*this)++; }

        // Get the current element or NULL and move backward
        Some Prev ()
        { return *(*this)--; }
    };
 ...
 * @arg VType
 *   HashVector class type name (SomeHashVector etc).
 * @arg CType
 *   The type of content items (SomeStruct etc). The vector is defined
 *   as if it contains pointers to SomeStruct.
 * @arg HashFunc
 *   Pointer to a hash function; if 0, the default hash function will be used
 * @arg Extra
 ...
 * @arg CType
 *   The type of content items (SomeStruct* etc). The vector is defined
 *   as if it contains this kind of items.
 * @arg KField
 *   The key field of the structure
 * @arg CmpExpr
 *   The expression to compare two keys, Key1 and Key2 and return -1, 0 or +1.
 */
