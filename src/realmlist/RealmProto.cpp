...
{
	length = sizeof (uint8) + sizeof (uint8) + sizeof (uint8) + ((ErrorCode == CE_SUCCESS) ? (sizeof (uint8)*32 + sizeof (uint8) + sizeof (uint8) + sizeof (uint8) + sizeof (uint8)*32 + sizeof (uint8)*32 + sizeof (uint8)*16 + sizeof (uint8)) : 0);
...
{
	length = sizeof (uint8) + sizeof (uint16) + sizeof (uint32) + sizeof (uint16) + 0;
	...
	*cur++ = SMSG_REALMLIST;
	...
	PUT_LE16 (cur, NumRealms); cur += sizeof (uint16);
	for (int i1 = 0; i1 < RealmList.Length(); i1++)
	{
