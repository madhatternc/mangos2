/**
 *     \file  src/worldserver/todo/NPCHandler.cpp
...
void NPCHandler::HandleMsg( NetworkPacket & recv_data, GameClient *pClient )
{
    ...
    switch (recv_data.opcode)
    {
        ...
        case CMSG_TRAINER_BUY_SPELL:
        {
            ...
            int price;
                                                            //guid for trainer
            guid tguid = pClient->getCurrentChar()->getSelection();
            recv_data >> guid1 >> guid2 >> spell_id;
            player_gold = pClient->getCurrentChar ()->getUpdateValue (PLAYER_FIELD_COINAGE);

                                                            //shouldn't get here handled client side
            if (pClient->getCurrentChar ()->isAllreadyLearned (spell_id))
                break;
