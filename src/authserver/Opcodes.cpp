...
bool CMSG_LOGON_CHALLENGE_t::Parse (Socket *socket)
{
	...
	if (!socket->GetLE (ClientIP)) return false;
	{
	    uint8 _sl;
...
{
	length = sizeof (uint8) + sizeof (uint8) + sizeof (uint8) + sizeof (uint8)*32 + sizeof (uint8) + sizeof (uint8) + sizeof (uint8) + sizeof (uint8)*32 + sizeof (uint8)*32 + sizeof (uint8)*16;

	data = new uint8 [length]; uint8 *cur = data;
	*cur++ = SMSG_LOGON_CHALLENGE;
	*cur = ASE_SUCCESS; cur += sizeof (uint8);
...
{
	ErrorCode = ASE_SUCCESS;
}

void SMSG_LOGON_PROOF_t::Assemble ()
{
	length = sizeof (uint8) + sizeof (uint8) + ((ErrorCode == ASE_SUCCESS) ? (sizeof (uint8)*20 + sizeof (uint32)) : 0);
...
void SMSG_REALMLIST_t::Assemble ()
{
	...
	for (int i1 = 0; i1 < RealmList.Length (); i1++)
		length += sizeof (uint32) + sizeof (uint8) + strlen (RealmList [i1]->Name) + 1 + strlen (RealmList [i1]->Address) + 1 + sizeof (float) + sizeof (uint8) + sizeof (uint8) + sizeof (uint8);
	length += 0 + sizeof (uint8) + sizeof (uint8);
	...
	*cur = NumRealms; cur += sizeof (uint8);
	for (int i1 = 0; i1 < RealmList.Length(); i1++)
	{
		PUT_LE32 (cur, RealmList [i1]->Type); cur += sizeof (uint32);
