/**
 *     \file  src/worldserver/todo/Unit.cpp
...
void Unit::UpdateMobMovement (uint32 p_time)
{
    ...
    if (m_creatureState == MOVING)
    {
        if (!m_moveTimer)
        {
            float dx, dy, dz;
                                                            // moveTimer interval
            float destMoved = m_moveSpeed*(UNIT_MOVEMENT_INTERPOLATE_INTERVAL + timediff);
            float moveDistance;
            ...
            else
            {
                ...
                //                UpdateObject();
                                                            ...
                                                            //creature is stoped
    else if (m_creatureState == STOPPED && !m_moveTimer && m_nWaypoints > 1)
    {
        int destpoint;
...
{
    Values->u32[ UNIT_NPC_EMOTESTATE ] = m_emoteState;
...
void Unit::AI_Update()
{
    ...
    for (itr = m_attackers.begin(); itr != m_attackers.end(); ++itr)
    {
        ...
        {
                                                            //LINA, force mobs to return to there respawn
            if (m_Pet == false) AI_MoveTo(respawn_cord[0][0], respawn_cord[0][1], respawn_cord[0][2], true);
        ...
        {
                                                            //test if in world
            if (!((Character*)pVictim)->pClient->IsInWorld())
            {
                                                            //LINA, force mobs to return to there respawn
                if (m_Pet == false) AI_MoveTo(respawn_cord[0][0], respawn_cord[0][1], respawn_cord[0][2], true);
                itr = m_attackers.erase(itr);
