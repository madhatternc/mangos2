/**
 *     \file  src/worldserver/todo/Level0.cpp
 *    \brief  Normal User Chat Commands.
 *
...
bool ChatHandler::HandleGMLoginCommand(uint8* args)
{
	...
	if (!*args)
		return false;
															...
															//fetches gm level
	int gm = WorldServer::getSingletonPtr( )->dbi->getAccountLvl(m_pClient->getAccountID());
															//fetches pass
	char * pass = WorldServer::getSingletonPtr( )->dbi->getAccountPass(m_pClient->getAccountID());
	if(!pass)
	{
...
bool ChatHandler::HandleMountCommand(uint8* args)
{
	NetworkPacket data;
															...
															// get level
	uint32 theLevel = m_pClient->getCurrentChar( )->getUpdateValue( UNIT_FIELD_LEVEL );
	uint16 mId=1147;
	float speed = (float)8;
